...
def parse_filter(parse_args):
    ...
    if parse_args.authors:
        ...
        for c_key in parse_args.authors.split(','):
            print(f"len of c_key {len(c_key)}")
            if len(c_key) <=64:
...
async def do_query(args):
    ...
    if args.authors:
        ...
        if isinstance(args.authors[0],str):            
            my_query['authors'] = [c_k for c_k in args.authors]
        ...
        # print(f"keys to use: {my_query['authors']}")
    async with ClientPool(args.relay.split(','),
        ...
        print(f"we got the events {len(events)}")
